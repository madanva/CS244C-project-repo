#!/usr/bin/env python3
"""
Run NCCL all-gather benchmark on 2 (or N) AWS EC2 GPU nodes, then terminate instances.
...
Flow:
  ...
  4. Run all_gather_perf for Ring (Simple, LL, LL128) and AUTO; optionally NVLS+Simple with --include-nvls.
     Save each run to a separate .txt in the results folder.
...
# AllGather supports only Ring (Simple, LL, LL128) and AUTO; optional NVLS+Simple via --include-nvls.

# PyTorch all-gather benchmark script (Modal-style: MASTER_ADDR/PORT, RANK, WORLD_SIZE).
# Runs on each process; reads env and prints nccl-tests-like table from rank 0.
# Set OVERLAP_MODE=1 to overlap compute (matmul) with all_gather; 0 or unset = sequential.
TORCH_ALLGATHER_BENCH_SCRIPT = r'''
import os
import sys
import time
import torch
...
def main():
    try:
        ...
        if rank == 0:
            ...
            lines.append("# PyTorch NCCL all_gather (env rendezvous, like Modal)")
        ...
        for size in sizes:
            ...
            tensor_list = [torch.empty(n, device=device, dtype=torch.float32) for _ in range(world_size)]
            if not overlap:
                ...
                # Overlap: run matmul on *default* stream; comm on separate stream.
                torch.cuda.synchronize()
            ...
            # AllGather: each rank receives size*(world_size-1) bytes
            algbw = size * (world_size - 1) / world_size / (elapsed_us * 1e3)
            busbw = size * (world_size - 1) / (elapsed_us * 1e3)
            if rank == 0:
...
def get_benchmark_configs(include_nvls: bool = False) -> list[tuple[str, dict[str, str] | None]]:
    """Return (tag, env_additions) for AllGather. AllGather supports only Ring (Simple, LL, LL128) and AUTO.
    If include_nvls, also run NVLS+Simple."""
    ...
    if include_nvls:
        configs.append(("nvls_simple", {"NCCL_ALGO": "NVLS", "NCCL_PROTO": "Simple"}))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run NCCL all-gather on 2 (or N) AWS EC2 GPU nodes, then terminate instances."
    )
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="DIR",
        help="Path to cached nccl-tests build (must contain all_gather_perf_mpi). Default: script's build_cache/nccl-tests-mpi/build",
    )
    ...
    parser.add_argument(
        "--include-nvls",
        action="store_true",
        help="Also run NVLS+Simple for AllGather (in addition to Ring Simple/LL/LL128 and AUTO). May fail on some topologies (e.g. 2-node 1-GPU TCP).",
    )
    ...
    try:
        ...
        if args.multinode_backend == "mpi":
            ...
            if not build_dir.is_dir() or not (build_dir / "all_gather_perf_mpi").is_file():
                sys.exit(
                    f"No valid cached build: '{build_dir}' is missing or does not contain all_gather_perf_mpi. "
                    "Build nccl-tests with MPI and all_gather_perf_mpi target, then pass --build-dir to this script."
                )
            print(f"Using cached MPI build from {build_dir}")
            remote_build = f"{remote_nccl}/build"
            local_binary = build_dir / "all_gather_perf_mpi"
            for ip in public_ips:
        ...
        master_port = 29500
        configs = [("auto", None)] if args.auto_only else get_benchmark_configs(include_nvls=args.include_nvls)
        if args.include_nvls and not args.auto_only:
            print("Including NVLS+Simple (--include-nvls).")
        ...
        if args.multinode_backend == "torch":
            ...
            script_path = SCRIPT_DIR / ".torch_allgather_bench.py"
            script_path.write_text(TORCH_ALLGATHER_BENCH_SCRIPT.strip())
            for ip in public_ips:
                scp_to_node(script_path, "/tmp/", ip, str(private_key_path))
            script_path.unlink(missing_ok=True)
            remote_script = "/tmp/.torch_allgather_bench.py"
            ...
            def run_one_config(tag: str, env_exports: str, overlap_mode: bool, out_suffix: str) -> None:
                ...
                out_file = results_dir / f"results_{total_gpus}gpu_allgather_{tag}{out_suffix}.txt"
                out_file.write_text(stdout)
                print(f"  [{tag}{out_suffix}] -> {out_file.name}")

            print(f"Running PyTorch NCCL all_gather for {len(configs)} configs (mode={args.mode})...")
            ...
            bench_args = "./build/all_gather_perf_mpi -b 8 -e 256M -f 2 -g 1"
            print(f"Running NCCL all_gather_perf for {len(configs)} configs{' (AUTO only)' if args.auto_only else ' (algo/proto + AUTO)'}...")
            for tag, env_additions in configs:
                if env_additions is None:
                    env_exports = "unset NCCL_ALGO NCCL_PROTO 2>/dev/null; "
                else:
                ...
                if result.returncode != 0:
                    ...
                    if result.stderr:
                        stdout = f"(stderr)\n{result.stderr}\n(stdout)\n{stdout}"
                out_file = results_dir / f"results_{total_gpus}gpu_allgather_{tag}.txt"
                out_file.write_text(stdout)
